...
REDFISH_MANAGER_BMC = "redfish/v1/Managers/bmc"
REDFISH_SYSTEM_BMC = "redfish/v1/Systems/system"
REDFISH_UBB_TASKS = "redfish/v1/TaskService/AMC/Tasks"
REDFISH_POWER_ENDPOINT = "redfish/v1/Chassis/PDB/PowerSubsystem/PowerSupplies"
BMC = "bmc"
BIOS = "bios"
BUNDLE_ACTIVE = "gpu"
PORT = 443
...
def check_power(bmc_ip, bmc_username, bmc_password, url):
    """
    Check a power supply and return False if health check failed
    """
    ...
    if not data:
        print(f"Power supply not found at {url}")
        return False

    name = data.get("Id", "Unknown PSU")
...
def check_power_supplies(bmc_ip, bmc_username, bmc_password):
    ...
    One of two host power supplies can fail and two of six UBB power supplies can fail.
    """

    # Motherboard PSUs
    psus = [
        f"{PROTOCOL}://{bmc_ip}:{PORT}/redfish/v1/Chassis/MB/PowerSubsystem/PowerSupplies/FRU_MB_PSU1",
        f"{PROTOCOL}://{bmc_ip}:{PORT}/redfish/v1/Chassis/MB/PowerSubsystem/PowerSupplies/FRU_MB_PSU2",
    ]

    failing_psu = 0

    for url in psus:
        if False == check_power(bmc_ip, bmc_username, bmc_password, url):
            failing_psu += 1

    # If more than 1 motherboard PSUs are failing
    if failing_psu > 1:
        log(
            "More than one host power supplies are missing connection or are faulty which is most likely causing system issues"
        )
    ...
    # UBB PSUs
    psus = [
        f"{PROTOCOL}://{bmc_ip}:{PORT}/redfish/v1/Chassis/PDB/PowerSubsystem/PowerSupplies/FRU_PDB_PSU1",
        f"{PROTOCOL}://{bmc_ip}:{PORT}/redfish/v1/Chassis/PDB/PowerSubsystem/PowerSupplies/FRU_PDB_PSU2",
        f"{PROTOCOL}://{bmc_ip}:{PORT}/redfish/v1/Chassis/PDB/PowerSubsystem/PowerSupplies/FRU_PDB_PSU3",
        f"{PROTOCOL}://{bmc_ip}:{PORT}/redfish/v1/Chassis/PDB/PowerSubsystem/PowerSupplies/FRU_PDB_PSU4",
        f"{PROTOCOL}://{bmc_ip}:{PORT}/redfish/v1/Chassis/PDB/PowerSubsystem/PowerSupplies/FRU_PDB_PSU5",
        f"{PROTOCOL}://{bmc_ip}:{PORT}/redfish/v1/Chassis/PDB/PowerSubsystem/PowerSupplies/FRU_PDB_PSU6",
    ]
    ...
    # If more than 2 UBB PSU are failing
    if failing_psu > 2:
        log(
            "More than two UBB power supplies are missing connection or are faulty which is most likely causing system issues"
        )
        print("\t1. Inspect power supplies")
        print(
            "\t2. Ensure all power supply connections are connected to active outlets"
